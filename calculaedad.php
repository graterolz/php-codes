	...
	<body>
		...
		<?php
			...
			// procesa los datos enviados
				...
				if(sizeof($fechaArr) != 3) die('ERROR: Por favor escriba una fecha válida');
				// verifica si los datos insertados son una fecha válida
				if(!checkdate($fechaArr[0],$fechaArr[1],$fechaArr[2])) die('ERROR: Por favor escriba una fecha de nacimiento válida');
				// valida que la fecha sea anterior a hoy, 'today'
				if ($fechaTs >= $now) die('ERROR: Por favor escriba una fecha anterior al día de hoy');
